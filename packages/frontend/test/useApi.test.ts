...
describe('useApi', () => {
  ...
  it('sets baseURL based on runtime config (dev)', () => {
    useRuntimeConfigMock.mockReturnValue({
      public: {
        apiEnvironment: 'dev',
        apiBaseUrlDev: 'http://dev-backend.internal:8080',
        apiBaseUrlProd: 'http://prod-backend.internal:8080',
      },
    })
    const api = useApi()
    expect(api.baseURL).toBe('http://dev-backend.internal:8080')
  })
  ...
  it('sets baseURL based on runtime config (prod)', () => {
    ...
    const api = useApi()
    expect(api.baseURL).toBe('http://prod-backend.internal:8080')
  })
